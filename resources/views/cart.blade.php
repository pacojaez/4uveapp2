        ...
        <div class="my-6 bg-white rounded shadow-md">
            @if(count($cart['products']) > 0)
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr>
                            <th class="px-6 py-4 text-sm font-bold uppercase border-b bg-grey-lightest text-grey-dark border-grey-light">Name</th>
                            <th class="px-6 py-4 text-sm font-bold uppercase border-b bg-grey-lightest text-grey-dark border-grey-light">Price</th>
                        ...
                        @foreach($cart['products'] as $product)
                            <tr class="hover:bg-grey-lighter">
                                ...
                                <td class="px-6 py-4 border-b border-grey-light">
                                    <a wire:click="removeFromCart({{ $product->id }})" class="px-3 py-1 text-xs font-bold text-green-600 rounded cursor-pointer bg-green hover:bg-green-dark">Remove</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
